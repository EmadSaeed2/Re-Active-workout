...
function Results(props) {
  ...
  useEffect(() => {
    fetch('https://api.pexels.com/v1/search?query=exercise&orientation=landscape&per_page=10', {
      method: "GET",
      headers: { Authorization: "hzZ9XK1o1pyeyFQaGTv4etjTC8cL2kgAlIy4B3khFlX5rO4hVdopMLoW" }
    })
  ...
  return (
    <main style={resultsCSS.main}>
      ...
      {exrData.map((data, index) => {
        return (
          <div className="resultCard" key={index}>
            <h3 className="resultData"> {data.name}</h3>
            <p> Target Muscle: <span className="resultData"> {data.muscle}</span></p>
            <p> Equipment used: <span className="resultData"> {data.equipment}</span></p>
            <p> Difficulty: <span className="resultData"> {data.difficulty} </span> </p>

            {/* <img src={imagesData.photos[index].src.landscape} alt="Re-Active Logo" style={resultsCSS.logoStyle} /> */}
